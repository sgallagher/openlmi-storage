...
class LMI_LocalFileSystem(CIMProvider2):
    ...
    def get_instance(self, env, model, device = None):
        ...
        name = model['Name']
        if device is None:
            device = storage.devicetree.getDeviceByPath(name)
        if device is None:
            raise pywbem.CIMError(pywbem.CIM_ERR_NOT_FOUND, 'Cannot find filesystem ' + model['Name'])

        #model['AvailableRequestedStates'] = [self.Values.AvailableRequestedStates.<VAL>,] # TODO 
        #model['AvailableSpace'] = pywbem.Uint64(device.block_size * filesystem.block_free)
        #model['BlockSize'] = pywbem.Uint64(device.block_size)
        #model['Caption'] = '' # TODO 
        #model['CasePreserved'] = bool() # TODO 
        #model['CaseSensitive'] = bool() # TODO 
        #model['ClusterSize'] = pywbem.Uint32() # TODO 
        #model['CodeSet'] = [self.Values.CodeSet.<VAL>,] # TODO 
        #model['CommunicationStatus'] = self.Values.CommunicationStatus.<VAL> # TODO 
        #model['CompressionMethod'] = '' # TODO 
        ...
        #model['EncryptionMethod'] = '' # TODO 
        #model['FileSystemSize'] = pywbem.Uint64(filesystem.block_size * filesystem.block_count)
        model['FileSystemType'] = device.format.name
        ...
        #model['MaxFileNameLength'] = pywbem.Uint32() # TODO 
        #model['NumberOfFiles'] = pywbem.Uint64() # TODO 
    ...
    class Values(object):
        ...
        class CodeSet(object):
            Unknown = pywbem.Uint16(0)
            Other = pywbem.Uint16(1)
            ASCII = pywbem.Uint16(2)
            Unicode = pywbem.Uint16(3)
            ISO2022 = pywbem.Uint16(4)
            ISO8859 = pywbem.Uint16(5)
            Extended_UNIX_Code = pywbem.Uint16(6)
            UTF_8 = pywbem.Uint16(7)
            UCS_2 = pywbem.Uint16(8)
