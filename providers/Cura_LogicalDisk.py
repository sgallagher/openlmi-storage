...
class Cura_LogicalDisk(CIMProvider2):
    ...
    def get_instance(self, env, model, device = None):
        ...
        if not logicalDiskManager.isExposed(device):
            raise pywbem.CIMError(pywbem.CIM_ERR_NOT_FOUND, "Device is not exposed as LogicalDisk.")

        model['BlockSize'] = pywbem.Uint64(device.partedDevice.sectorSize) 
        model['Name'] = path
        model['NumberOfBlocks'] = pywbem.Uint64(device.partedDevice.length)
        return model
    ...
    def enum_instances(self, env, model, keys_only):
        ...
        for device in storage.devices:
            if logicalDiskManager.isExposed(device) and not isinstance(device, pyanaconda.storage.devices.LVMLogicalVolumeDevice):
